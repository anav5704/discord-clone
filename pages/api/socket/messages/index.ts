...
export default async function handler(req: NextApiRequest, res: NextApiResponeServerIO) {
    ...
    try {
        ...
        if (!serverId) return res.status(400).json({ error: "Server id is mising" })
        if (!channelId) return res.status(400).json({ error: "Channel id is mising" })
        if (!content) return res.status(400).json({ error: "Content is mising" })
